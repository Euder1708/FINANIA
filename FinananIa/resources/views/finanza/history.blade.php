                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @foreach($q as $transaction)
                                        <tr class="hover:bg-gray-50">
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                <div>
                                                    <div class="font-medium">{{ \Carbon\Carbon::parse($transaction->occurred_at)->format('d/m/Y') }}</div>
                                                    <div class="text-xs text-gray-500">{{ \Carbon\Carbon::parse($transaction->occurred_at)->format('H:i') }}</div>
                                                </div>
                                            </td>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium
                                                ...
                                                {{ $transaction->type === 'credit' ? '+' : '-' }}{{ number_format($transaction->amount, 2) }} USD
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
